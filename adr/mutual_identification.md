...
- **For EBW-to-EBW traffic**, this decision does not fix the prefix. WE BUILD shall select, before interoperability testing, at least one prefix that every EBW must support, so that any EBW can be reached by any other. Candidates are `x509_hash`, `verifier_attestation`, `openid_federation` and `decentralized_identifier`. Additional prefixes may be used where both parties support them.
